...
def test_organization_repository(db_session):
    ...
    org = repo.create(db_session, org_data)
    ...
    # Test get
    retrieved_org = repo.get(db_session, org.id)
    ...
    active_orgs = repo.get_active_organizations(db_session)
    ...
    upserted_org = repo.upsert(db_session, upsert_data)
...
def test_office_repository(db_session):
    ...
    org = org_repo.create(db_session, {"name": "Test Organization", "is_active": True})
    ...
    office = office_repo.create(db_session, office_data)
    assert office.id is not None
    assert office.name == "Test Office"
    assert office.organization_id == org.id

    # Test get
    retrieved_office = office_repo.get(db_session, office.id)
    ...
    updated_office = office_repo.update(db_session, db_obj=office, obj_in=update_data)
    ...
    active_offices = office_repo.get_active_offices(db_session)
    assert len(active_offices) == 1
    assert active_offices[0].id == office.id

    # Test get_by_organization
    org_offices = office_repo.get_by_organization(db_session, org.id)
    assert len(org_offices) == 1
    assert org_offices[0].id == office.id

    # Test get_by_coordinates
    nearby_offices = office_repo.get_by_coordinates(db_session, 41.7, 44.8, 20.0)
    ...
    upserted_office = office_repo.upsert(db_session, upsert_data)
    ...
    new_office = office_repo.upsert(db_session, new_office_data)
    assert new_office.id != office.id
    assert new_office.name == "New Office"

    # Test mark_inactive_if_not_in_list
    count = office_repo.mark_inactive_if_not_in_list(db_session, [new_office.id])
    assert count == 1  # One office should be marked as inactive

    # Verify the first office is now inactive
    inactive_office = office_repo.get(db_session, office.id)
    assert inactive_office.is_active is False

    # Test delete
    deleted_office = office_repo.delete(db_session, id=new_office.id)
    assert deleted_office.id == new_office.id
    assert office_repo.get(db_session, new_office.id) is None
...
def test_rate_repository(db_session):
    ...
    rate = rate_repo.create(db_session, rate_data)
    ...
    updated_rate = rate_repo.update(db_session, db_obj=rate, obj_in=update_data)
    assert updated_rate.id == rate.id
    assert updated_rate.buy_rate == 2.67
    assert updated_rate.sell_rate == 2.72

    # Test get_latest_rates
    latest_rates = rate_repo.get_latest_rates(db_session)
    assert len(latest_rates) == 2

    # Test get_latest_rates with filters
    usd_rates = rate_repo.get_latest_rates(db_session, currency="USD")
    assert len(usd_rates) == 1
    assert usd_rates[0].currency == "USD"

    office_rates = rate_repo.get_latest_rates(db_session, office_id=office.id)
    assert len(office_rates) == 2

    # Test get_rates_by_office
    office_rates = rate_repo.get_rates_by_office(db_session, office.id)
    assert len(office_rates) == 2

    # Test get_rates_by_currency
    usd_rates = rate_repo.get_rates_by_currency(db_session, "USD")
    assert len(usd_rates) == 1
    assert usd_rates[0].currency == "USD"

    # Test get_best_rates
    best_buy_rates = rate_repo.get_best_rates(db_session, "USD", buy=True)
    ...
    upserted_rate = rate_repo.upsert(db_session, upsert_data)
    ...
    new_rate = rate_repo.upsert(db_session, new_rate_data)
    ...
    old_rate = rate_repo.create(db_session, old_rate_data)

    # Now delete old rates
    deleted_count = rate_repo.delete_old_rates(db_session, hours=3)
    assert deleted_count == 1  # One rate should be deleted

    # Verify the old rate is gone
    assert rate_repo.get(db_session, old_rate.id) is None

    # Test delete
    deleted_rate = rate_repo.delete(db_session, id=new_rate.id)
    assert deleted_rate.id == new_rate.id
    assert rate_repo.get(db_session, new_rate.id) is None
