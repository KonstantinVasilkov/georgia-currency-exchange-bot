...
async def test_get_latest_rates_table_full_scenario(mock_repos):
    ...
    org_repo.find_one_by.side_effect = lambda **kwargs: next((o for o in orgs if o.external_ref_id == kwargs.get('external_ref_id')), None)
    ...
    def make_rates(org_name):
        if org_name == "NBG":
            return [MagicMock(currency="USD", buy_rate=2.5), MagicMock(currency="EUR", buy_rate=3.0), MagicMock(currency="RUB", buy_rate=0.03)]
        if org_name == "Bank of Georgia":
            return [MagicMock(currency="USD", buy_rate=2.6), MagicMock(currency="EUR", buy_rate=3.1), MagicMock(currency="RUB", buy_rate=0.031)]
        if org_name == "TBC Bank":
            return [MagicMock(currency="USD", buy_rate=2.7), MagicMock(currency="EUR", buy_rate=3.2), MagicMock(currency="RUB", buy_rate=0.032)]
        if org_name == "Credo Bank":
            return [MagicMock(currency="USD", buy_rate=2.8), MagicMock(currency="EUR", buy_rate=3.3), MagicMock(currency="RUB", buy_rate=0.033)]
        if org_name == "BestOrg1":
            return [MagicMock(currency="USD", buy_rate=2.9), MagicMock(currency="EUR", buy_rate=3.4), MagicMock(currency="RUB", buy_rate=0.034)]
        if org_name == "BestOrg2":
            return [MagicMock(currency="USD", buy_rate=2.4), MagicMock(currency="EUR", buy_rate=2.9), MagicMock(currency="RUB", buy_rate=0.029)]
        return []
    rate_repo.get_rates_by_office.side_effect = lambda office_id, limit=10: make_rates(next(o.name for o in orgs if o.id == office_id))
